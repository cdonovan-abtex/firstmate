...
test_pi_snippet_uses_effective_extension_path() {
  ...
  assert_contains "$out" "That exact request-bound turn is not an ordinary wake-handling turn" "pi snippet lost the outcome-processing exception to ordinary wake draining"
  assert_contains "$out" "must not drain the wake queue again" "pi snippet did not forbid a second drain during exact outcome processing"
  assert_contains "$out" "process those outcomes before any unrelated operation" "pi snippet lost request-bound processing priority"
  assert_contains "$out" "MAIN must not re-drain, re-run, or acknowledge it" "pi snippet lost merged-event ownership"
  assert_contains "$out" "does not replace any captain-facing outcome response" "pi snippet allowed the visible anchor to replace the response"
  assert_contains "$out" "not just \`Captain, shipshape.\`" "pi snippet allowed a completion result to collapse to a routine acknowledgement"
