...
PY
  ...
  while IFS='|' read -r mutation case_name diagnostic; do
    out=$(FM_CONTEXT_TEST_EMITTER="$TMP_ROOT/mutant.py" FM_CONTEXT_MUTATION="$mutation" \
      bash "${BASH_SOURCE[0]}" --case "$case_name" 2>&1)
    [ "$?" -ne 0 ] || fail "$mutation mutant survived its behavioral test"
