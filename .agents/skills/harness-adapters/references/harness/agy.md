...
| Composer | A `>` row between full-width rules, followed by the idle footer; the shared classifier requires a live AGY identity plus that complete shape before proving empty input. |
