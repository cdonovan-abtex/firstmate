...
Propagation failures normally warn without blocking secondmate launch or session-start continuation, and the destination keeps whatever safely validated state the helper left behind.
The mechanically enforced `config/no-mistakes-policy.json` is the local-launch exception: `fm-spawn.sh` requires byte-exact source/destination convergence, or matching absence, before creating the secondmate endpoint, so a skipped, failed, stale, symlinked, or hardlinked policy cannot open an unguarded child-dispatch gap.
...
The instruction uses only minimal framing that most items are defaults or rules and that `config/no-mistakes-policy.json` is a mechanically enforced exception; it never includes SHA values, selected profiles, parsed summaries, or any other generated interpretation.
...
Most inherited config values remain defaults and rules only; `config/no-mistakes-policy.json` is the deliberate exception because its positive agent allowlist and shared-service ceiling are mechanically enforced at the worker validation boundary.
The policy never constrains the outer worker harness, model, effort, or runtime backend.
