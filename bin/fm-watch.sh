...
# the separate idle absorb case for both signal and stale paths and re-surfaces
...
# A current authoritative paused state uses the bounded cadence regardless of
# endpoint liveness. A stale paused/captain-held log is recovered only when the
# agent is confidently dead; ambiguous stale logs still surface once.
...
# The watcher records the captain-relevant status line it SURFACED (woke
# firstmate for) in .hb-surfaced-<task>, the watcher's analogue of the daemon's
...
EOF
    ...
    #   - or it is a no-verb wake (a bare turn-end, a working: note) whose crew has
    #     neither an actively-running pipeline/busy pane nor an authoritative
    #     declared pause. Such a crew may be done (even via an interactive menu
    #     that wrote no done: status), waiting on a decision, or wedged.
    # Actionable -> enqueue, advance .seen-* markers, exit. Benign (a no-verb wake
    # whose crew IS working or paused) in always-on mode -> advance the markers so
    # it will not re-fire, log, and keep blocking without enqueuing. The absorb-class
    # check is the only costly one (it may run a bounded no-mistakes call), so the ||
    # ordering evaluates it ONLY for a non-afk, no-captain-verb signal.
    # shellcheck disable=SC2086  # $files is a space-separated status-path list (ids carry no spaces)
    if afk_present || signal_reason_is_actionable $files || ! signal_crews_absorbable $files; then
      while IFS=$(printf '\t') read -r sf sig f; do
        [ -n "$sf" ] || continue
