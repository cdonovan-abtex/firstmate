...
test_lost_scout_work_refuses_and_faulty_absence_classifier_is_detected
case_dir="$TMP_ROOT/lost-scout-work"
id=lost-scout-unpreserved-commit
printf '\nProduction guard output (exit 1 asserted by fixture):\n'
cat "$case_dir/stderr"
write_sparse_recovery_meta "$case_dir" "$id" no-mistakes scout "$case_dir/wt"
printf '\nRepeat the same refusal expectation against the faulty executable:\n'
rc=0
run_teardown_for "$case_dir/faulty-bin/fm-teardown.sh" "$case_dir" "$id" > "$case_dir/mutation.stdout" 2> "$case_dir/mutation.stderr" || rc=$?
cat "$case_dir/mutation.stdout"
red=0
(expect_code 1 "$rc" 'Lost-work refusal must reject the faulty absence-is-empty guard') || red=$?
expect_code 1 "$red" 'Counterfactual refusal assertion must turn red'
assert_absent "$case_dir/state/$id.meta" 'Faulty guard must expose record removal'
printf 'Counterfactual assertion exit: %s (expected failure). Faulty guard removed recovery metadata.\n' "$red"
