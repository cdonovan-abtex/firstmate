...
test_empty_scout_uses_fresh_remote_default_branch
test_lost_scout_work_refuses_and_faulty_absence_classifier_is_detected
python3 - "$TMP_ROOT" <<'CAPTURE'
from pathlib import Path
import sys
root=Path(sys.argv[1])
out=Path('/Users/christiandonovan/.no-mistakes/evidence/01M25ZDC899VCS736C4111NH64/cleanup-cli-transcript.txt')
with out.open('w') as f:
    f.write('Real bin/fm-teardown.sh executed against isolated Git repositories and recovery metadata.\nGitHub responses and endpoint/treehouse operations are fixture-controlled; tasks-axi is real.\n\n')
    for case in sorted(root.iterdir()):
        if not case.is_dir(): continue
        f.write('CASE '+case.name+'\n')
        for p in sorted(case.iterdir()):
            if p.name in ('stdout','stderr','faulty.out','faulty.err','pr-api.json') or p.suffix in ('.stdout','.stderr'):
                f.write('--- '+p.name+' ---\n'+p.read_text()+'\n')
        meta=list((case/'state').glob('*.meta'))
        f.write('Persisted task records after case: '+(', '.join(p.name for p in meta) or '(none)')+'\n')
        backlog=case/'data/backlog.md'
        if backlog.exists(): f.write('Persisted backlog:\n'+backlog.read_text()+'\n')
        f.write('\n')
CAPTURE
