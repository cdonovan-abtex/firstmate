...
id=reports-canonical-malish-credential-vault-v1
write_sparse_recovery_meta "$case_dir" "$id" direct-PR ship ''
printf 'pr=%s\npr_head=%s\n' 'https://github.com/abtex/abtex-epicor-reports/pull/43' '55b3ad333f8a6d9078a1f9f8ff9abde0620da834' >> "$case_dir/state/$id.meta"
git -C "$case_dir/project" remote set-url origin https://github.com/abtex/abtex-epicor-reports.git
cat > "$case_dir/fakebin/gh-axi" <<'CLI'
#!/usr/bin/env bash
exec /Users/christiandonovan/.gh-tools/lib/node_modules/gh-axi/dist/bin/gh-axi.js "$@"
CLI
chmod +x "$case_dir/fakebin/gh-axi"
rm "$case_dir/fakebin/gh"
printf 'Live GitHub API; isolated metadata/project; simulated endpoint/treehouse operations.\n'
printf '$ bin/fm-teardown.sh %s\n' "$id"
rc=0
run_teardown_for "$TEARDOWN" "$case_dir" "$id" || rc=$?
printf 'Exit status: %s\n' "$rc"
expect_code 0 "$rc" 'Live Reports PR43 cleanup'
assert_absent "$case_dir/state/$id.meta" 'Live Reports PR43 retained record'
printf 'Persisted state: recovery metadata removed.\n'
