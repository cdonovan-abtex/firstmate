...
. "/Users/christiandonovan/.no-mistakes/worktrees/d9c4fb60435f/01M23C8SWJK71BKJ6J4T67RG5W/tests/lib.sh"
...
EVIDENCE_DIR=/Users/christiandonovan/.no-mistakes/evidence/01M23C8SWJK71BKJ6J4T67RG5W

# Capture the actual executable output and resulting task/backlog state.
eval "$(declare -f run_teardown_for | sed '1s/run_teardown_for/original_run_teardown_for/')"
run_teardown_for() {
  local tool=$1 case_dir=$2 id=$3 rc capture
  capture=$(mktemp -d "$case_dir/capture.XXXXXX")
  {
    printf '\n$ %s %s' "$tool" "$id"
    local arg
    for arg in "${@:4}"; do printf ' %s' "$arg"; done
    printf '\nBefore: recovery record\n'
    cat "$case_dir/state/$id.meta"
  } >> "$EVIDENCE_DIR/supplemental-cli-transcript.log"
  if original_run_teardown_for "$@" > "$capture/stdout" 2> "$capture/stderr"; then rc=0; else rc=$?; fi
  {
    cat "$capture/stdout" "$capture/stderr"
    printf 'Exit code: %s\n' "$rc"
    if [ -e "$case_dir/state/$id.meta" ]; then printf 'After: recovery record retained\n'; else printf 'After: recovery record removed\n'; fi
    if [ -f "$case_dir/data/backlog.md" ]; then
      printf 'Persisted backlog:\n'
...
# End-to-end sparse cleanup using the installed gh-axi and live read-only API.
case_dir=$(make_case live-reports-pr43)
id=reports-canonical-malish-credential-vault-v1
write_sparse_recovery_meta "$case_dir" "$id" direct-PR ship ''
printf '%s\n' 'pr=https://github.com/abtex/abtex-epicor-reports/pull/43' >> "$case_dir/state/$id.meta"
git -C "$case_dir/project" remote set-url origin https://github.com/abtex/abtex-epicor-reports.git
cat > "$case_dir/fakebin/gh-axi" <<'SH'
#!/usr/bin/env bash
[ "${1:-}" = api ] || exit 1
exec /Users/christiandonovan/.gh-tools/lib/node_modules/gh-axi/dist/bin/gh-axi.js "$@"
SH
chmod +x "$case_dir/fakebin/gh-axi"
# gh-axi delegates to gh; let the live check resolve the installed client.
rm "$case_dir/fakebin/gh"
run_teardown_for "$TEARDOWN" "$case_dir" "$id" || fail 'live PR 43 teardown refused'
assert_absent "$case_dir/state/$id.meta" 'live PR 43 recovery record retained'
pass 'live gh-axi API confirms PR 43 and real teardown removes its sparse recovery record'

# An existing isolated scout copy with unpreserved commits must remain intact.
case_dir=$(make_case existing-unpreserved-scout)
id=unpreserved-scout-copy
write_sparse_recovery_meta "$case_dir" "$id" no-mistakes scout "$case_dir/wt"
wt_commit_file "$case_dir" irreplaceable.txt 'unpreserved scout work'
expected_head=$(git -C "$case_dir/wt" rev-parse HEAD)
rc=0
run_teardown_for "$TEARDOWN" "$case_dir" "$id" || rc=$?
expect_code 1 "$rc" 'unpreserved scout copy must refuse'
assert_present "$case_dir/state/$id.meta" 'unpreserved record removed'
assert_equals "$expected_head" "$(git -C "$case_dir/wt" rev-parse HEAD)" 'unpreserved commit changed'
assert_grep 'unpreserved scout work' "$case_dir/wt/irreplaceable.txt" 'unpreserved content lost'
printf 'Verified after refusal: commit %s and irreplaceable.txt remain intact.\n' "$expected_head" >> "$EVIDENCE_DIR/supplemental-cli-transcript.log"
pass 'existing unpreserved scout commit and content retained'

# Reproduce both latest accepted regressions against the immediate pre-fix guard.
previous_bin="$TMP_ROOT/previous-bin"
mkdir -p "$previous_bin"
for helper in "$ROOT/bin"/*; do ln -s "$helper" "$previous_bin/$(basename "$helper")"; done
rm "$previous_bin/fm-teardown.sh"
git -C "$ROOT" show f7dcdd0:bin/fm-teardown.sh > "$previous_bin/fm-teardown.sh"
chmod +x "$previous_bin/fm-teardown.sh"
for regression in test_empty_scout_checks_index_assumptions test_sparse_recovery_backlog_with_legacy_option; do
  rc=0
  (TEARDOWN="$previous_bin/fm-teardown.sh"; "$regression") > "$EVIDENCE_DIR/before-fix-$regression.log" 2>&1 || rc=$?
  expect_code 1 "$rc" "pre-fix $regression must fail"
  pass "pre-fix regression reproduced: $regression"
done
