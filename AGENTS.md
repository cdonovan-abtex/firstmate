...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  handoffs/<artifact>/  durable captain-facing handoff artifacts (section 9); an artifact child is never a task id, and no `report.md` may sit directly at `handoffs/report.md`
...
Before giving the captain any localhost or Lavish URL, browser-open that exact URL and prove the intended content rendered plus one safe reversible non-feedback interaction where applicable through a fresh task-owned browser bridge on a unique `CHROME_DEVTOOLS_AXI_PORT` with auto-connect, external-browser-URL, and shared-profile modes disabled, then stop only that bridge on its own port immediately afterward; never verify through the captain's active browser, and never treat an HTTP 200 response or editor-shell markup as proof.
Publish captain-facing Lavish HTML only under the operator-owned vault `90_AI_Exports/<topic>/`, a repository's durable `.lavish/` that sits outside every disposable task copy, or `$FM_HOME/data/handoffs/<artifact>/`, never `/private/tmp/claude-501/.../scratchpad/`, the wider `/private/tmp/claude-*/.../scratchpad/` class, generic `/tmp/`, a disposable task copy, or a disposable `.lavish/`.
