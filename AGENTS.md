...
Use that exact branch evidence when telling the captain the PR's full URL, concise outcome summary, and no-mistakes risk level when applicable; say "merged into `<base>`" rather than default-branch delivery unless the repository default is established and matches.
