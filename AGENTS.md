...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  handoffs/<artifact>/  durable captain-facing handoff artifacts; handoffs is a reserved layout name that must never be used as a task id (docs/configuration.md; publication policy in section 9)
...
Label each handoff `LIVE` with a named owner actively running `lavish-axi poll` who transfers ownership or completes the `decision-hold-lifecycle` gate before ending the review.
Label it `STATIC` when no owner will watch it, state in the artifact itself that it is non-interactive and that annotations reach nobody, and leave no open unwatched session behind.
If a `LIVE` handoff's owner disappears while its session is still open, treat any prompt already pending there as unread captain input and escalate immediately, because an input channel nobody reads is worse than no input channel.
