...
Before giving the captain any localhost or Lavish URL, verify the intended content rendered plus a safe reversible non-feedback interaction where applicable through a fresh task-owned browser bridge on a unique `CHROME_DEVTOOLS_AXI_PORT` with auto-connect, external browser URL, and shared-profile modes disabled; never verify through the captain's active browser, never treat an HTTP 200 response or editor-shell markup as proof, and withhold the URL when that fresh bridge cannot be established.
Publish a Firstmate-managed captain-facing Lavish artifact under a dedicated artifact-named child of `$FM_HOME/data/` or another pre-existing operator-owned durable root that current local instructions name explicitly, never `/private/tmp/claude-*/.../scratchpad/`, generic `/tmp/`, an isolated task copy, or a disposable `.lavish/`; label it `LIVE` with a named owner actively running `lavish-axi poll` who transfers ownership or completes the `decision-hold-lifecycle` gate before ending the review, or `STATIC` with an explicit warning that annotations reach nobody and no open unwatched session left behind.
Treat any pending Lavish prompt that the owning startup, fleet, or harvest mechanism surfaces without a live poll owner as unread captain input requiring immediate escalation, because an input channel nobody reads is worse than no input channel.
