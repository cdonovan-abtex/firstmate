...
config/no-mistakes-policy.json  optional default-off positive validation-agent allowlist and machine-shared service concurrency ceiling; LOCAL, gitignored, and inherited by secondmate homes; docs/configuration.md owns the schema and scope
...
Firstmate workers use only the bare `no-mistakes` command supplied in their PATH, whose optional agent allowlist and machine-shared concurrency policy is owned by `bin/fm-no-mistakes.mjs` and documented in `docs/configuration.md`.
