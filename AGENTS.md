...
Before giving the captain any localhost or Lavish URL, first prove the browser tool is detached from the captain's active browser rather than attached through auto-connect, an existing browser URL, or a shared profile directory, then verify the intended content rendered using only a safe reversible non-feedback interaction where applicable; an HTTP 200 response or editor-shell markup is never proof, and when detachment cannot be proven, do not navigate and do not hand off the URL.
A Firstmate-managed captain-facing Lavish artifact is a durable handoff published under `$FM_HOME/data/<id>/` or another pre-existing operator-owned durable export root that current local instructions name explicitly; never infer or create an undocumented root, and never publish from a per-conversation path such as `/private/tmp/claude-*/.../scratchpad/`, generic `/tmp/`, an isolated task copy, or any disposable `.lavish/`.
Label every handoff `LIVE` or `STATIC`: `LIVE` names the surviving owner actively running `lavish-axi poll` and, before that owner exits, either transfers ownership or ends the review only after the `decision-hold-lifecycle` completion gate, while `STATIC` states plainly that annotations reach nobody and leaves no open unwatched session.
A pending prompt that the session-start or fleet-view owner surfaces without a live poll owner is unread captain input that must be escalated immediately, because an input channel nobody reads is worse than no input channel; deterministic scanning and its integration into those owners belong to the separate queued `firstmate-lavish-pending-sweep-v1`, which this line never redefines.
