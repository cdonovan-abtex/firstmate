...
Before giving the captain any localhost or Lavish URL, browser-open that exact URL and prove the intended content rendered plus one safe reversible non-feedback interaction where applicable through a fresh task-launched Chrome for Testing browser or installed Chromium with a distinct application-bundle identity, an isolated profile or context, and a task-owned bridge on a unique `CHROME_DEVTOOLS_AXI_PORT` with auto-connect, external-browser-URL, and shared-profile modes disabled; never launch `/Applications/Google Chrome.app` for automation even headless, never verify through the captain's active browser, never borrow the captain's browser, profile, tabs, or debugging endpoint, and never treat an HTTP 200 response or editor-shell markup as proof; record the exact owned browser, context, and process handles at launch, close only those exact handles and that bridge on its own port immediately afterward, never use a blanket Chrome quit, broad `pkill`, or AppleScript stop, and park verification without navigation or cleanup when isolation and ownership cannot be proven.
Publish captain-facing Lavish HTML only under the operator-owned vault `90_AI_Exports/<topic>/`, a repository's durable `.lavish/` after the artifact has landed through that project's selected delivery path, or the dedicated non-task namespace `$FM_HOME/data/handoffs/<artifact>/`, never by writing under `projects/` and never from `/private/tmp/claude-501/.../scratchpad/`, the wider `/private/tmp/claude-*/.../scratchpad/` class, generic `/tmp/`, an unlanded or disposable task copy, or a disposable `.lavish/`.
Label each handoff `LIVE` with a named owner actively running `lavish-axi poll` who transfers ownership or completes the `decision-hold-lifecycle` gate before ending the review, or `STATIC` with an explicit warning that annotations reach nobody and no open unwatched session left behind, and treat any pending prompt without a live owner as unread captain input requiring immediate escalation, because an input channel nobody reads is worse than no input channel.
