...
Keep the `Co-Authored-By` trailer naming the model that produced the work, and never strip it.
It records provenance, not credit: the commit author stays the accountable human, while the trailer tells a later reader which model wrote this and how much to weigh it.
...
If `state/.afk-contract` or `state/.afk` is present, load `/afk` in away mode or `/quiet` in quiet mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`) and follow that skill's supervision procedure.
...
At the start of every ordinary wake-handling turn, drain the durable wake queue before peeking, reading beyond the reason line, steering, or starting work.
Session start is one exception because its one-shot digest already presented the queue while locked or deliberately left it untouched in lock-refused read-only mode.
An exact Pi supervision outcome-processing request is the other exception: the listed fleet events are already handled, so follow the emitted Pi protocol and process only those listed outcomes before any unrelated operation without draining again.
...
- In away mode, bias ambiguous input toward exit because a present captain takes precedence.
