...
A crew that declares `paused:` for a known external wait is absorbed while idle on both the signal and stale paths, including the pause's initial no-verb status signal in normal mode, and re-surfaced only on the longer `FM_PAUSE_RESURFACE_SECS` cadence rather than treated as a possible wedge; away mode likewise self-handles that routine signal and owns the later recheck.
For an ordinary crew that has stopped, an authoritative current `paused` state uses that same bounded cadence regardless of endpoint liveness, while a stale `paused:` or durable `captain-held` status log with no authoritative pause first surfaces one stale wake and is recovered to the cadence only when the backend confidently reports its agent dead; live or inconclusive liveness remains fail-open at that initial stale-log surface, and the secondmate idle-endpoint exemption is unchanged.
[`architecture.md`](architecture.md#event-driven-supervision) owns how long that classification is reused before crew state and endpoint liveness are read again.
