...
The first two presentations of a given sequence set trigger main immediately, queued as a follow-up while main is busy.
If both are ignored, one 60-second delayed presentation triggers main once more without a captain prompt; if that fixed third attempt is also ignored, one turn-free visible note says automatic continuation did not finish, the outcome remains open, and a fresh store read injects it at every later `before_agent_start` prompt boundary.
This three-attempt total is fixed rather than operator-expandable: it prevents an ignored request from silently becoming prompt-dependent after only the immediate attempts without turning model noncompliance into a periodic spend loop.
It deliberately does not queue a Pi `nextTurn` snapshot: changed sequence membership cancels the older set's delayed timer and starts its own fresh bounded processing turn immediately, while a session replacement also starts that budget over.
Routine outcomes never enter this path, schedule no timer, and stay turn-free.
...
`FM_PI_RESULT_CONTINUATION_HERDR_E2E=1 tests/fm-pi-result-continuation-herdr-live-e2e.test.sh`, with locally supplied approved Pi provider and model selectors, starts a real Pi in an isolated named Herdr lab, withholds the action tools for the first two processing turns, and proves that the delayed third turn performs the synthetic completion's authorized isolated next action and exact acknowledgement without a user prompt.
