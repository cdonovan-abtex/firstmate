...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is neither provably working nor authoritatively paused, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign when `bin/fm-crew-state.sh` reports the crew authoritatively working - an actively running no-mistakes step attributed to that crew's current code or an exact busy verdict from the semantic busy-state contract - or authoritatively paused on a declared external wait, in either case only while it has no still-open decision the captain has not already been woken for.
Because a later `working:` or `paused:` line appended after a `needs-decision:` or `blocked:` masks that decision in any last-line read, both the working and paused absorb classes consult the authoritative `status_open_decisions` fold and its exactly-once decision-seen marker, surfacing once whenever an unsurfaced decision remains open.
A crew that declares `paused:` for a known external wait is separately absorbed while idle on both the signal and stale paths and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
For an ordinary crew that has stopped, an authoritative current `paused` state uses that same bounded cadence regardless of endpoint liveness, while a stale `paused:` or durable `captain-held` status log with no authoritative pause first surfaces one stale wake and is recovered to the cadence only when the backend confidently reports its agent dead.
Each pause classification is then cached for at most `FM_STALE_ESCALATE_SECS`: while that recheck marker is fresh, a still-`paused:`/`captain-held` log stays on the cadence without re-reading authoritative crew state or endpoint liveness at all, so benign pane churn from a live agent inside that bounded window is absorbed instead of re-surfaced.
Once the marker expires, the full reclassification above runs again, so a crew that left its pause is recovered within one window rather than held quiet indefinitely.
Live or inconclusive liveness remains fail-open at that initial stale-log surface, and the secondmate idle-endpoint exemption is unchanged.
Registered checks and genuinely actionable new `needs-decision:`/`blocked:` events reach the captain on their own paths throughout the cached window, so the cache narrows only wedge-style stale re-surfacing.
The declared pause's initial no-verb status signal is absorbed in normal mode rather than surfaced, and away mode likewise self-handles that routine signal and owns the later recheck.
...
The daemon escalates captain-relevant events, an open decision masked by a later `working:` or `paused:` line, plus a bounded recheck for a declared pause that remains idle, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
