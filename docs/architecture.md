...
It distinguishes open review from a closed-without-merging or locked result rather than folding them together, and a forge it cannot read at all is reported as unavailable while arming still records the canonical identity and arms the watch.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records the canonical PR identity and any available destination, repository-default, and head evidence through `bin/fm-pr-check.sh` before calling the forge CLI.
