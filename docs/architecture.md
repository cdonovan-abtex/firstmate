...
`bin/fm-pr-poll.sh` is the single extractor and formatter for both ready and merged outcomes: it reads the forge-reported base and repository default, states when they differ, and explicitly leaves default-branch delivery unverified when either value is unavailable.
The receipt makes retirement safely retryable across restarts: fixed-path recovery revalidates the same evidence, removes the runnable check first, removes its registration and data sidecars, removes the receipt last, and preserves task metadata including `pr=`, `pr_base=`, `pr_default=`, and `pr_head=`.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_base=`, `pr_default=`, and `pr_head=` evidence through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
