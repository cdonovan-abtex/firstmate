...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only while `bin/fm-crew-state.sh` reports the crew authoritatively working - an actively running no-mistakes step attributed to that crew's current code or an exact busy verdict from the semantic busy-state contract - or authoritatively paused on a declared external wait, and only while no decision the captain has not been woken for remains open; [watcher-continuity.md](watcher-continuity.md#notification-boundary) owns those absorb classes, the masked-decision fold with its exactly-once decision-seen marker, and the endpoint-liveness rules that recover a stale `paused:` or `captain-held` log.
Each pause classification is then cached for at most `FM_STALE_ESCALATE_SECS`: while that recheck marker is fresh, a still-`paused:`/`captain-held` log stays on the pause cadence without re-reading authoritative crew state or endpoint liveness at all, so benign pane churn from a live agent inside that bounded window is absorbed instead of re-surfaced, and once the marker expires the full reclassification runs again so a crew that left its pause is recovered within one window rather than held quiet indefinitely.
