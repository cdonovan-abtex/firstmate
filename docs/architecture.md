...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign when `bin/fm-crew-state.sh` reports the crew authoritatively working - an actively running no-mistakes step attributed to that crew's current code or a backend busy signature - or deliberately paused on a declared external wait with no still-open decision.
Because a `paused:` line appended after a `needs-decision:` or `blocked:` masks that decision in any last-line read, the paused absorb consults the authoritative `status_open_decisions` fold and surfaces instead whenever the fold is non-empty.
