...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign when `bin/fm-crew-state.sh` reports the crew authoritatively working - an actively running no-mistakes step attributed to that crew's current code or a backend busy signature - or deliberately paused on a declared external wait.
