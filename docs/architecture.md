...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign when `bin/fm-crew-state.sh` reports the crew authoritatively working - an actively running no-mistakes step attributed to that crew's current code or a backend busy signature - or deliberately paused on a declared external wait, in either case only while it has no still-open decision the captain has not already been woken for.
...
A crew that declares `paused:` for a known external wait is absorbed while idle on both the signal and stale paths and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
