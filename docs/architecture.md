...
GitHub check evaluation is owned by `bin/fm-pr-merge.sh`'s `github_checks_not_green`; older non-green rollup entries still block merging unless explicitly waived, because a later passing run with the same display name does not prove shared workflow or provider lineage.
...
For a live pool slot, the owner claim written under the allocation lock and owned by [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) must prove both the task and its canonical home; a mismatched, absent, or unprovable claim makes teardown refuse before cleanup, preserving the slot and task records even with `--force`.
