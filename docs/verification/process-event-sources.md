...
| rebind-all refreshes eligible tracked actions only | `tests/fm-procevent-when.test.sh` commits the changed regular executable before rebinding, verifies it fires once and a repeated rebind changes nothing, and preserves an external action's binding; ignored, untracked, symlink, and dirty tracked action controls retain their old bindings across an unrelated update and reject the changed action without claiming a fire |
...
| primary and secondmate updates preserve watch bindings | `tests/fm-update.test.sh` updates the primary while a watch is already polling and verifies the new action runs once; `tests/fm-secondmate-sync.test.sh` drives local and remote home sync through the shared boundary, verifying dirty-home refusals preserve HEAD, work, and trust before a clean retry advances and fires the new action |
| registration serializes with updates | `tests/fm-procevent-when.test.sh` pauses each side of the transaction in both arm-first and update-first order, then verifies the newly registered watch executes the updated action once; unavailable-action and missing-target refusals leave no partial registration or changed HEAD and permit a successful retry |
