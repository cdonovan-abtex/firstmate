...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and durable captain-facing handoff artifacts in named `data/handoffs/<artifact>/` children.
Because per-task records live at `data/<id>/`, `handoffs` is a reserved layout name that must never be used as a task id.
Task-id creation validation (`fm_task_id_creation_valid` in `bin/fm-pr-lib.sh`) does not yet reject it, so spawning a task named `handoffs` would write a brief and scout report straight into the handoff namespace; enforcing the reservation in code is follow-up work.
Each artifact keeps its files inside its own `<artifact>/` child rather than directly at `data/handoffs/`, and `AGENTS.md` section 9 owns whether a captain-facing artifact may be published there at all.
