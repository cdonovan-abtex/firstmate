...
The foreground arm legitimately follows a healthy watcher until its next wake, so the hook catches HUP, TERM, and INT from host timeout or teardown through the ordinary durable failure handoff, subject to the same current supervision-need, away/quiet-mode, generation-ownership, and consumed-alarm suppression checks.
...
`tests/fm-pi-watch-extension.test.sh` covers both Pi markers through the startup digest with absent and previous-session locks, without pre-arming or reloading the extensions, plus descendant rejection and the missing-extension negative control; `tests/fm-turnend-guard.test.sh` covers direct-owner publication, reload, session replacement, foreign-owner refusal, and state-directory isolation.
